...
func HTTPAPIServerStreamSaveToMP4NoDel(c *gin.Context) {
	var err error
	...
	requestLogger := log.WithFields(logrus.Fields{
		...
		"stream":  c.Param("uuid"),
		"channel": c.Param("channel"),
		"func":    "HTTPAPIServerStreamSaveToMP4NoDel",
	})
	...
	go func() {
		...
		path := fmt.Sprintf("../SNOVA-backend/static/no-autodel-record/%s/%s", c.Param("uuid"), c.Param("channel"))
